...
//! looks it up. The `56 XxxRenderer` traits themselves stay public and
//! unchanged — they are the **type-level** contract a theme implements,
//! the HashMap is the **storage** layer.
//!
//! ## Why not 56 named fields?
//!
//! The 56-trait design (one trait per component) is the right level of
...
//! 200+-method union (because the 56 method sets are largely disjoint
//! — `Switch` has `track_w/track_h/knob_size` while `Avatar` has
//! `status_dot_size/status_inset`).
//!
//! What the 56-field `RendererRegistry` *didn't* need was the named
...
//! public surface is the 56 `with_<x>` / 56 `get_<x>` method pairs.

use std::any::{Any, TypeId};
use std::collections::HashMap;
use std::sync::Arc;

use super::avatar::WinUIAvatarRenderer;
use super::badge::WinUIBadgeRenderer;
use super::button::WinUIButtonRenderer;
use super::button_group::WinUIButtonGroupRenderer;
use super::card::WinUICardRenderer;
use super::checkbox::WinUICheckboxRenderer;
use super::combo_box::WinUIComboBoxRenderer;
use super::disclosure::WinUIDisclosureRenderer;
use super::divider::WinUIDividerRenderer;
use super::dropdown_menu::WinUIDropdownMenuRenderer;
use super::empty_state::WinUIEmptyStateRenderer;
use super::file_path_input::WinUIFilePathInputRenderer;
use super::focus_ring::WinUIFocusRingRenderer;
use super::form::WinUIFormRenderer;
use super::form_field::WinUIFormFieldRenderer;
use super::grid_view::WinUIGridViewRenderer;
use super::heading::WinUIHeadingRenderer;
use super::icon::WinUIIconRenderer;
use super::icon_button::WinUIIconButtonRenderer;
use super::image::WinUIImageRenderer;
use super::keybinding_display::WinUIKeybindingDisplayRenderer;
use super::keybinding_input::WinUIKeybindingInputRenderer;
use super::label::WinUILabelRenderer;
use super::list_item::WinUIListItemRenderer;
use super::listbox::WinUIListboxRenderer;
use super::menu::WinUIMenuRenderer;
use super::modal::WinUIModalRenderer;
use super::notification::WinUINotificationRenderer;
use super::number_input::WinUINumberInputRenderer;
use super::overlay::WinUIOverlayRenderer;
use super::panel::WinUIPanelRenderer;
use super::password_input::WinUIPasswordInputRenderer;
use super::popover::WinUIPopoverRenderer;
use super::progress::WinUIProgressBarRenderer;
use super::radio::WinUIRadioRenderer;
use super::radio_group::WinUIRadioGroupRenderer;
use super::search_input::WinUISearchInputRenderer;
use super::select::WinUISelectRenderer;
use super::shortcut_hint::WinUIShortcutHintRenderer;
use super::skeleton::WinUISkeletonRenderer;
use super::slider::WinUISliderRenderer;
use super::spacer::WinUISpacerRenderer;
use super::split_button::WinUISplitButtonRenderer;
use super::switch::WinUISwitchRenderer;
use super::table::WinUITableRenderer;
use super::tag::WinUITagRenderer;
use super::text::WinUITextRenderer;
use super::text_area::WinUITextAreaRenderer;
use super::text_input::WinUITextInputRenderer;
use super::toast::WinUIToastRenderer;
use super::toggle_button::WinUIToggleButtonRenderer;
use super::tooltip::WinUITooltipRenderer;
use super::tree::WinUITreeRenderer;
use super::tree_item::WinUITreeItemRenderer;
use super::uniform_virtual_list::WinUIUniformVirtualListRenderer;
use super::virtual_list::WinUIVirtualListRenderer;
...
use yororen_ui_core::renderer::grid_view::GridViewRenderState;
...
///
/// Public surface: 56 `with_<x>(Arc<dyn XxxRenderer>)` setters + 56
...
impl RendererRegistry {
    /// All renderers set to the default `WinUIXxxRenderer` implementations.
    ...
    /// between the 56 setters and the 56 `REQUIRED` entries.
    pub fn token_based() -> Self {
        ...
        // the empty registry directly and chain the 56 setters.
        let registry = Self {
            map: HashMap::new(),
        }
        .with_button(Arc::new(WinUIButtonRenderer))
        .with_button_group(Arc::new(WinUIButtonGroupRenderer))
        .with_icon_button(Arc::new(WinUIIconButtonRenderer))
        .with_toggle_button(Arc::new(WinUIToggleButtonRenderer))
        .with_label(Arc::new(WinUILabelRenderer))
        .with_heading(Arc::new(WinUIHeadingRenderer))
        .with_icon(Arc::new(WinUIIconRenderer))
        .with_divider(Arc::new(WinUIDividerRenderer))
        .with_focus_ring(Arc::new(WinUIFocusRingRenderer))
        .with_badge(Arc::new(WinUIBadgeRenderer))
        .with_tag(Arc::new(WinUITagRenderer))
        .with_progress_bar(Arc::new(WinUIProgressBarRenderer))
        .with_skeleton(Arc::new(WinUISkeletonRenderer))
        .with_slider(Arc::new(WinUISliderRenderer))
        .with_tooltip(Arc::new(WinUITooltipRenderer))
        .with_avatar(Arc::new(WinUIAvatarRenderer))
        .with_switch(Arc::new(WinUISwitchRenderer))
        .with_checkbox(Arc::new(WinUICheckboxRenderer))
        .with_radio(Arc::new(WinUIRadioRenderer))
        .with_text_input(Arc::new(WinUITextInputRenderer))
        .with_text_area(Arc::new(WinUITextAreaRenderer))
        .with_password_input(Arc::new(WinUIPasswordInputRenderer))
        .with_number_input(Arc::new(WinUINumberInputRenderer))
        .with_file_path_input(Arc::new(WinUIFilePathInputRenderer))
        .with_search_input(Arc::new(WinUISearchInputRenderer))
        .with_select(Arc::new(WinUISelectRenderer))
        .with_combo_box(Arc::new(WinUIComboBoxRenderer))
        .with_modal(Arc::new(WinUIModalRenderer))
        .with_popover(Arc::new(WinUIPopoverRenderer))
        .with_dropdown_menu(Arc::new(WinUIDropdownMenuRenderer))
        .with_disclosure(Arc::new(WinUIDisclosureRenderer))
        .with_toast(Arc::new(WinUIToastRenderer))
        .with_notification(Arc::new(WinUINotificationRenderer))
        .with_panel(Arc::new(WinUIPanelRenderer))
        .with_card(Arc::new(WinUICardRenderer))
        .with_form(Arc::new(WinUIFormRenderer))
        .with_form_field(Arc::new(WinUIFormFieldRenderer))
        .with_grid_view(Arc::new(WinUIGridViewRenderer))
        .with_list_item(Arc::new(WinUIListItemRenderer))
        .with_listbox(Arc::new(WinUIListboxRenderer))
        .with_menu(Arc::new(WinUIMenuRenderer))
        .with_overlay(Arc::new(WinUIOverlayRenderer))
        .with_radio_group(Arc::new(WinUIRadioGroupRenderer))
        .with_spacer(Arc::new(WinUISpacerRenderer))
        .with_table(Arc::new(WinUITableRenderer))
        .with_text(Arc::new(WinUITextRenderer))
        .with_tree(Arc::new(WinUITreeRenderer))
        .with_tree_item(Arc::new(WinUITreeItemRenderer))
        .with_virtual_list(Arc::new(WinUIVirtualListRenderer))
        .with_uniform_virtual_list(Arc::new(WinUIUniformVirtualListRenderer))
        .with_keybinding_input(Arc::new(WinUIKeybindingInputRenderer))
        .with_split_button(Arc::new(WinUISplitButtonRenderer))
        .with_empty_state(Arc::new(WinUIEmptyStateRenderer))
        .with_image(Arc::new(WinUIImageRenderer))
        .with_keybinding_display(Arc::new(WinUIKeybindingDisplayRenderer))
        .with_shortcut_hint(Arc::new(WinUIShortcutHintRenderer));
    ...
    renderer_setter!(with_grid_view, GridViewRenderState, GridViewRenderer);
    ...
    // -- 56 typed accessors (one per component) -------------------------
    ...
    renderer_getter!(get_grid_view, GridViewRenderState, GridViewRenderer);
    ...
    /// All 56 `(TypeId, "name")` pairs that a complete registry must
    ...
    const REQUIRED: &[(TypeId, &str)] = &[
        ...
        (TypeId::of::<GridViewRenderState>(), "grid_view"),
    ...
    /// Verify that this registry contains a renderer for **all** 56
    ...
    match registry.validate() {
        ...
        Err(missing) => panic!(
            "RendererRegistry::token_based() is missing {} renderer(s): {}. \
             This is an internal bug: token_based() must register all 56 components. \
...
mod tests {
    ...
    fn only_button() -> RendererRegistry {
        ...
        let arc: Arc<dyn ButtonRenderer> = Arc::new(super::super::button::WinUIButtonRenderer);
        map.insert(
    ...
    fn empty_registry_reports_all_missing() {
        ...
        assert_eq!(
            err.len(),
            56,
            "expected 56 missing entries, got {}: {:?}",
            err.len(),
            err
        );
    ...
    fn override_after_token_based_still_passes() {
        ...
        let r = RendererRegistry::token_based()
            .with_button(Arc::new(super::super::button::WinUIButtonRenderer));
        r.validate()
    ...
    fn token_based_panics_if_a_setter_call_is_removed() {
        ...
        // by constructing an almost-complete registry (55 of 56) and
